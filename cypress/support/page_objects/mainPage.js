export class mainPage {
    ...
    fillUpContactUsForm(contactName, contactEmail, contactPhone, contactSubject, contactDescription) {
        cy.get('.container-fluid').find('[class="row contact"]').then(contactUs => {
           if(contactName !== null) cy.get(onMainPage.contactNameField).type(contactName)
           if (contactEmail !== null) cy.get(onMainPage.contactEmailField).type(contactEmail)
           if (contactPhone !== null) cy.get(onMainPage.contactPhoneField).type(contactPhone)
           if (contactSubject !== null) cy.get(onMainPage.contactSubjectField).type(contactSubject)
           if(contactDescription !== null) cy.get(onMainPage.contactDescriptionField).type(contactDescription)
